...
my_df <- mod_met_filtered %>% 
      filter(mod_met_filtered$sn == 'MOD-00024')
...
pollutants <- c('pm10', 'pm25', 'pm1', 'no', 'no2', 'co', 'o3')
...
for (p in pollutants) {
  polarPlot(my_df, pollutant = p, main=p)
  polarPlot(my_df, pollutant = p, type = "season",  main=p)
  polarPlot(my_df, pollutant = p, limits = c(0,6), type = "season",  main=p)
...
for (p in pollutants) {
  calendarPlot(my_df, pollutant = "pm10", main = p)
  trendLevel(my_df, pollutant = "pm10", limits = c(0,20), main=p)
}

# calendarPlot(my_df, pollutant = "pm10", limits = c(0,80))
# 
# 
calendarPlot(my_df, pollutant = "pm10", annotate = "value", limits = c(0,80), lim =50, cols = "Purples", col.lim = c("black", "orange"), layout = c(4, 3))
